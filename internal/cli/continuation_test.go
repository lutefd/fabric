...
func TestContinueValidationAndNoActiveDirection(t *testing.T) {
	...
	if _, ok := threads["thread-empty"]; !ok {
		t.Fatal("empty continuation thread was not persisted")
	}
}
...
func TestContinueBudgetPrioritizesReviewDirection(t *testing.T) {
	...
	mustRun(t, "continue", "--pr", "123", "--budget", "20")
	...
	assertDelivered(t, "thread-pr", recordIDAt(t, 1))
}
